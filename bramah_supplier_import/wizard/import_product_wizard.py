...
class BramahProductImport(models.TransientModel):
    _name = "bramah.product.import"
    _description = "Bramah Product Import"
    ...
    def _import_record_data(self, import_file):
        ...
        for row in range(1, sheet.nrows):
            ...
            if name:
                ...
                if product_template:
                    print("*"*80)
                    print("product_template:", product_template.id, product_template.name)
                else:
                    print("*"*80)
                    print("No existe producto:", name, ref)
    ...
    def _search_product_template(self, name, ref):
        ...
        product_template = self.env["product.template"].search(
            [("name", "=", name.strip()), ("default_code", "=", ref.strip())]
        )
        if product_template:
            print("*"*80)
